...
const updateUserFormData = (params) => {
  ...
  formData.append(
    "avatar",
    params.data.avatar != null ? `$avatar_${user.id}_${user.avatar}` : ""
  );
  params.data?.birthDate
    ? formData.append("birthDate", params.data.birthDate)
    : formData.append("birthDate", params.data.previousData?.birthDate);
  params.data?.phone
    ? formData.append("phone", params.data.phone)
    : formData.append(
        "phone",
        !params.data.previousData?.phone ? "" : params.data.previousData?.phone
      );
  ...
  params.data?.emailConfirm &&
    formData.append("emailConfirm", params.data.emailConfirm);

  return formData;
};

const messageImageFromData = (params) => {
  const formData = new FormData();
  const files = params.data.messageImageDto.images;
  const messageDto = params.data.messageImageDto.messageDto;
...
const productFormData = (params, files = []) => {
  const formData = new FormData();
    ...
    if (obj?.url) {
      ...
      const fetchImage = async () => {
        try {
          ...
          const blob = await response.blob();
          const file = new File([blob], obj.image, {
            type: blob.type,
          });
          file["path"] = obj.image;
          data.push(file);
